...
export default function BillingConsumptionPage() {
  ...
  const averageConsumption = totalBills ? totalConsumption / totalBills : 0;

  const consumptionByWoreda = useMemo(() => {
    ...
    return Array.from(grouped.entries())
      .map(([woredaId, value]) => ({ woredaId, ...value }))
    ...
    <div className="text-[#e8f4f0]">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          ...
          ["Average Consumption", averageConsumption, "m³ per bill"],
          ["Bills Count", totalBills, "bills included"],
        ].map(([label, value, sub]) => (
          <div
            key={label}
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="border-b border-[rgba(29,158,117,0.06)]">
                  {["Woreda", "Bills", "Total Consumption"].map((header) => (
                    <th
                      key={header}
