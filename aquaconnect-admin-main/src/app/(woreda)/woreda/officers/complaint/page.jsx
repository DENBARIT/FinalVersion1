...
export default function ComplaintOfficersPage() {
  ...
  const complaintOfficers = useMemo(() => {
    return allOfficers
      .filter((o) => o.fieldOfficerType === "COMPLAINT_OFFICER")
      .filter((o) => {
        const matchSearch =
          !search ||
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(complaintOfficers.length / PAGE_SIZE),
  );
  const officers = complaintOfficers.slice(
    (page - 1) * PAGE_SIZE,
    page * PAGE_SIZE,
  );
