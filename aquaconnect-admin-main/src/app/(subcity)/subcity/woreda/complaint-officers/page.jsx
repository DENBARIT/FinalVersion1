...
export default function WoredaComplaintOfficersPage() {
  ...
  const refreshOfficers = async () => {
    const rows = await superAdminService.getComplaintOfficers({ subCityId });
    const normalized = Array.isArray(rows) ? rows : [];
    const woredaComplaintRows = normalized.filter(
      (item) => item.role === "WOREDA_COMPLAINT_OFFICER",
    );

    const rowsWithAssignments = await Promise.all(
      woredaComplaintRows.map(async (item) => {
        try {
          const assignmentsPayload =
            await superAdminService.getWoredaComplaintOfficerAssignments(
              item.id,
            );
  ...
  const handleEdit = async (data) => {
    ...
    try {
      ...
      await refreshOfficers();
      setEditTarget(null);
      showToast("success", "Woreda complaint officer updated successfully.");
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast("success", "Woreda complaint officer deleted successfully.");
    } catch (err) {
      const message = String(err?.message || "").toLowerCase();
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      showToast(
        "success",
        `Woreda complaint officer ${nextStatus.toLowerCase()} successfully.`,
      );
    } catch (err) {
      const message = String(err?.message || "").toLowerCase();
  ...
  const handleExportCsv = () => {
    ...
    link.href = url;
    link.download = "woreda-complaint-officers.csv";
    link.click();
  ...
  const handleAssignWoredas = async () => {
    ...
    try {
      await superAdminService.assignWoredaComplaintOfficer(
        assignTarget.id,
        selectedWoredaIds,
      );
      await refreshOfficers();
  ...
  return (
    <div className="text-[#e8f4f0] bg-[#05141f] border border-[rgba(29,158,117,0.08)] rounded-2xl overflow-hidden">
          ...
          <tbody>
            ...
            {!loading && !officers.length && (
              <tr>
                ...
                >
                  No woreda complaint officers found for your subcity.
                </td>
              </tr>
            )}
          </tbody>
      ...
      <Modal
        ...
        title="Edit Woreda Complaint Officer"
      >
        <SubcityOfficerForm
          onSubmit={handleEdit}
      ...
      <ConfirmModal
        ...
        loading={loading}
        title="Delete Woreda Complaint Officer"
      ...
      <ConfirmModal
        ...
        title={
          statusTarget?.status === "SUSPENDED"
            ? "Activate Woreda Complaint Officer"
            : "Suspend Woreda Complaint Officer"
        }
